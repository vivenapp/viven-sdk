...
namespace TwentyOz.VivenSDK.Scripts.Core.Lua
{
    ...
    public class VivenLuaBehaviour : MonoBehaviour
    {
        ...
        /// <summary>
        /// Lua Behaviour에서 편집할 수 있는 Injection 정보
        /// </summary>
        public UIInjectionInfo[] uiInjectionInfoList;

        /// <summary>
        /// Lua 스크립트로 전달된 Injection 정보
        /// </summary>
        [SerializeField]
        private InjectedFieldList newInjection;

        // Reused during refresh to avoid per-call dictionary allocations.
        private readonly Dictionary<Type, HashSet<string>> _validNamesByTypeCache = new Dictionary<Type, HashSet<string>>();

    #region Injection

        /// <summary>
        /// Legacy Injection의 변경사항을 newInjection에 반영합니다.
        /// Inspector에서 Legacy Injection이 편집되었을 때 호출됩니다.
        /// </summary>
        public void ApplyInjections()
        {
            ApplyInjectionToInjectedField(overwriteExisting: true);
        }

        private void ApplyInjectionToInjectedField(bool overwriteExisting)
        {
            UpsertInjectionValues(injection.objectValues, typeof(Object), overwriteExisting, value => value?.name,
                value => value != null ? (object)value.value : null);
            UpsertInjectionValues(injection.gameObjectValues, typeof(GameObject), overwriteExisting, value => value?.name,
                value => value != null ? (object)value.value : null);
            UpsertInjectionValues(injection.vector3Values, typeof(Vector3), overwriteExisting, value => value?.name,
                value => value != null ? (object)value.value : null);
            UpsertInjectionValues(injection.floatValue, typeof(float), overwriteExisting, value => value?.name,
                value => value != null ? (object)value.value : null);
            UpsertInjectionValues(injection.intValue, typeof(int), overwriteExisting, value => value?.name,
                value => value != null ? (object)value.value : null);
            UpsertInjectionValues(injection.boolValue, typeof(bool), overwriteExisting, value => value?.name,
                value => value != null ? (object)value.value : null);
            UpsertInjectionValues(injection.stringValue, typeof(string), overwriteExisting, value => value?.name,
                value => value != null ? value.value : null);
            UpsertInjectionValues(injection.colorValue, typeof(Color), overwriteExisting, value => value?.name,
                value => value != null ? (object)value.value : null);
            UpsertInjectionValues(injection.vivenScriptValue, typeof(VivenScript), overwriteExisting,
                value => value?.name,
                value => value != null ? (object)value.value : null);
        }

        private void UpsertInjectionValues<TValue>(TValue[] sourceValues, Type storageType, bool overwriteExisting,
            Func<TValue, string> getName, Func<TValue, object> getValue)
        {
            if (sourceValues == null || sourceValues.Length == 0) return;

            for (var i = 0; i < sourceValues.Length; i++)
            {
                var source = sourceValues[i];
                var fieldName = getName(source);
                if (string.IsNullOrEmpty(fieldName)) continue;

                if (!overwriteExisting && newInjection.TryGetField(storageType, fieldName, out _))
                {
                    continue;
                }

                newInjection.Upsert(storageType, fieldName, getValue(source));
            }
        }

        /// <summary>
        /// 새로운 VivenScript가 할당될 때 호출합니다.
        /// 기존 newInjection 데이터를 초기화하고 새 스크립트 기준으로 재구성합니다.
        /// </summary>
        /// <remarks>
        /// 스크립트 변경 시에만 사용합니다. 동일 스크립트의 재임포트에는 <see cref="RefreshParsedInjectionInfos"/>를 사용하세요.
        /// </remarks>
        public void InitializeInjections(VivenScript newScript)
        {
            luaScript = newScript;
            EnsureInjectionFieldList();
            SyncInjectionsWithParsedInfos();
        }

        private void ApplyParsedInjectionList()
        {
            if (luaScript?.parsedInjectionInfos == null) return;
            // Parsed Injection 목록으로 데이터를 초기화함.
            foreach (var parsed in luaScript.parsedInjectionInfos)
            {
                var storageType = GetStorageType(parsed?.Type);
                var fieldName = parsed?.name;
                if (storageType == null || string.IsNullOrEmpty(fieldName)) continue;

                // 데이터가 이미 존재한다면 초기화 X
                if (newInjection.TryGetField(storageType, fieldName, out var result)) continue;
                // 데이터가 없는 경우 null로 초기화
                newInjection.Upsert(storageType, fieldName, null);
            }
        }

        private void InitializeUIInjectionInfo()
        {
            if (luaScript?.parsedInjectionInfos == null)
            {
                uiInjectionInfoList = Array.Empty<UIInjectionInfo>();
                return;
            }

            var parsedInfos = luaScript.parsedInjectionInfos;
            if (uiInjectionInfoList == null || uiInjectionInfoList.Length != parsedInfos.Length)
            {
                uiInjectionInfoList = new UIInjectionInfo[parsedInfos.Length];
            }

            for (var i = 0; i < parsedInfos.Length; i++)
            {
                uiInjectionInfoList[i] ??= new UIInjectionInfo();
                var parsedInfo = parsedInfos[i];
                var storageType = GetStorageType(parsedInfo?.Type);
                uiInjectionInfoList[i].Type = parsedInfo?.Type;
                uiInjectionInfoList[i].name = parsedInfo?.name;
                uiInjectionInfoList[i].TargetField = null;
                if (storageType != null && newInjection.TryGetField(storageType,
                        parsedInfo?.name, out var result))
                {
                    uiInjectionInfoList[i].TargetField = result;
                }
            }
        }

        public void RefreshUIInjectionList()
        {
            if (uiInjectionInfoList == null) return;
            foreach (var uiInfo in uiInjectionInfoList)
            {
                if (uiInfo.TargetField != null) continue;
                var storageType = GetStorageType(uiInfo.Type);
                if (storageType == null || string.IsNullOrEmpty(uiInfo.name)) continue;

                if (newInjection.TryGetField(storageType, uiInfo.name, out var result))
                {
                    uiInfo.TargetField = result;
                }
            }
        }

        /// <summary>
        /// 동일 스크립트가 재임포트되었거나, 역직렬화 후 참조 복원이 필요할 때 호출합니다.
        /// 기존 newInjection에 저장된 사용자 값을 보존하면서 parsedInfos 변경사항을 반영합니다.
        /// </summary>
        public void RefreshParsedInjectionInfos(VivenScript refreshedScript)
        {
            if (refreshedScript == null) return;

            luaScript = refreshedScript;
            EnsureInjectionFieldList();
            SyncInjectionsWithParsedInfos();
        }

        /// <summary>
        /// Legacy Injection과 Parsed Injection 정보를 newInjection에 동기화하고 UI 배열을 재구성합니다.
        ///
        /// 처리 순서:
        /// 1. 유효 이름 수집: Legacy Injection + parsedInfos에서 유효한 (Type, name) 쌍을 수집
        /// 2. 정리: newInjection에서 유효 이름에 포함되지 않는 필드를 제거
        /// 3. Legacy 반영: Legacy Injection의 값을 newInjection에 복사 (이미 존재하는 필드는 건너뜀)
        /// 4. Parsed 반영: parsedInfos에 선언되었으나 newInjection에 없는 필드를 null로 생성
        /// 5. UI 구성: parsedInfos 기준으로 uiInjectionInfoList를 재구성하고 newInjection의 필드와 연결
        ///
        /// Legacy Injection은 독립적으로 동작하며, UI에는 Parsed(checkInject/NullableInject) 기준으로만 표시됩니다.
        /// Legacy에서 InjectedField로의 마이그레이션은 LegacyInjectionMigrator를 사용하세요.
        /// </summary>
        private void SyncInjectionsWithParsedInfos()
        {
            if (luaScript == null || newInjection == null) return;

            // 1. 유효 이름 수집
            var validNamesByType = CollectInjectionNamesByType();
            var parsedInfos = luaScript.parsedInjectionInfos;
            if (parsedInfos != null)
            {
                for (var i = 0; i < parsedInfos.Length; i++)
                {
                    var parsed = parsedInfos[i];
                    if (parsed == null || string.IsNullOrEmpty(parsed.name)) continue;
                    var storageType = GetStorageType(parsed.Type);
                    if (storageType == null) continue;
                    AddName(storageType, parsed.name, validNamesByType);
                }
            }

            // 2. 정리
            newInjection.RemoveFieldsNotIn(validNamesByType);

            // 3. Legacy 반영 (기존 값 보존, 새 필드만 추가)
            ApplyInjectionToInjectedField(overwriteExisting: false);

            // 4. Parsed 반영 (누락된 필드를 null로 생성)
            ApplyParsedInjectionList();

            // 5. UI 구성 (Parsed + Legacy 전용 필드 포함)
            InitializeUIInjectionInfo();
        }

        private Dictionary<Type, HashSet<string>> CollectInjectionNamesByType()
        {
            var names = _validNamesByTypeCache;
            foreach (var set in names.Values)
            {
                set.Clear();
            }
            if (injection == null) return names;

            AddInjectionNames(typeof(Object), injection.objectValues, names, v => v?.name);
            AddInjectionNames(typeof(GameObject), injection.gameObjectValues, names, v => v?.name);
            AddInjectionNames(typeof(Vector3), injection.vector3Values, names, v => v?.name);
            AddInjectionNames(typeof(float), injection.floatValue, names, v => v?.name);
            AddInjectionNames(typeof(int), injection.intValue, names, v => v?.name);
            AddInjectionNames(typeof(bool), injection.boolValue, names, v => v?.name);
            AddInjectionNames(typeof(string), injection.stringValue, names, v => v?.name);
            AddInjectionNames(typeof(Color), injection.colorValue, names, v => v?.name);
            AddInjectionNames(typeof(VivenScript), injection.vivenScriptValue, names, v => v?.name);

            return names;
        }

        private void EnsureInjectionFieldList()
        {
            newInjection ??= new InjectedFieldList();
        }

        private static void AddInjectionNames<T>(Type valueType, T[] values,
            Dictionary<Type, HashSet<string>> namesByType, Func<T, string> getName)
        {
            ...
            for (var i = 0; i < values.Length; i++)
            {
                ...
                AddName(valueType, name, namesByType);
            }
        }

        private static void AddName(Type valueType, string name,
            Dictionary<Type, HashSet<string>> namesByType)
        {
            if (valueType == null || string.IsNullOrEmpty(name)) return;
            if (!namesByType.TryGetValue(valueType, out var names))
            {
                names = new HashSet<string>();
                namesByType[valueType] = names;
            }
        ...
        /// <summary>
        /// 주어진 타입을 newInjection이 지원하는 저장 타입으로 변환합니다.
        /// 지원하지 않는 타입이면 null을 반환합니다.
        /// </summary>
        private Type GetStorageType(Type type)
        {
            if (type == null) return null;
            var normalizedType = ParsedInjectionInfo.NormalizeStorageType(type);
            return newInjection.IsValidType(normalizedType) ? normalizedType : null;
        }

        [ContextMenu("Print Injections")]
        public void PrintInjections()
        {
            Debug.Log("Printing injections: " + gameObject.name);
            newInjection.Print();
        }
