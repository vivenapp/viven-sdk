...
namespace TwentyOz.VivenSDK.Scripts.Core.Common
{
    ...
    public static class VivenAPI
    {
        ...
        {
            return UnityWebRequest.PostWwwForm($"{VivenDomain.API.GetDomainAPI(domain)}/client/auth/logout", string.Empty);
        }
        ...
        {
            var request = UnityWebRequest.Get($"{VivenDomain.API.GetDomainAPI(domain)}/client/auth/me");
        ...
        {
            return UnityWebRequest.Post($"{VivenDomain.API.GetDomainAPI(domain)}/client/auth/login", form);
        }
    }
}